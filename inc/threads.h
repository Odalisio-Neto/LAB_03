...
void *Ref(void *args)
{
    ...
    while (t <= TEMPO_MAX * 1000)
    {
        ...
        printf("x_ref : %.4f y_ref : %.4f, t: %lf, ts1 : %lf , TMAX: %lf \n",
               VALUES(bufferRef, 0, 0), VALUES(bufferRef, 1, 0), t, ts1.tv_nsec, TEMPO_MAX * 1000);

        bufferRef = defineRef(tm); // verificar t = t*T ()
